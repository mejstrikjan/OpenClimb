...
export async function insertAscent(ascent: {
  ...
  await db.runAsync(
    `INSERT INTO ascents (id, route_id, date, style, success, notes, created_at, updated_at, synced)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0)`,
    [id, ascent.route_id, ascent.date, ascent.style, ascent.success ? 1 : 0, ascent.notes, now, now]
  );
  return id;
}
